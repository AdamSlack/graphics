...
GLuint uniformbuffer,vertexbuffer,colourbuffer,normalbuffer,tcoordbuffer,tangentbuffer,bitangentbuffer;
...
//		mytexturesize_index=glGetUniformLocation( glContext0, "TexMapSize" );
		...
		glGenBuffers(1, &tangentbuffer);//allocate name
			GLGETERROR("GenBuffers");
...
int LoadPolys( FILE *infile )
{
	...
	{
		fgets( cInString, 1000, infile);						 // Read first/next line of file
		sscanf( cInString, "%d%d%d", &polylist[iNumPolys].nv,&polylist[iNumPolys].texindex, &polylist[iNumPolys].normindex);   // Get number of vertices, texture index, and normal indexS
